...
namespace PTT_NGROUR.Models.DataModel
{
    ...
    {
        public ModelOMAccumulated() { }
        public ModelOMAccumulatedPipeline Pipeline { get; set; }
        public ModelOMAccumulatedMaintenanceLevel Gate { get; set; }
        public ModelOMAccumulatedMaintenanceLevel Meter { get; set; }

        public class ModelOMAccumulatedPipeline
        {
            public ModelOMAccumulatedPipeline(int month, IEnumerable<ModelMonitoringResults> listResults, string mode)
            {
                Results = listResults.ToList();

                if (!month.Equals(0))
                {
                    listResults = listResults.Where(x => !string.IsNullOrEmpty(x.PM_TYPE));
        ...
        public class ModelOMAccumulatedMaintenanceLevel
        {
            public ModelOMAccumulatedMaintenanceLevel(int month, int year, IEnumerable<ModelMonitoringResults> listResults, string mode)
            {
                Results = listResults.ToList();
